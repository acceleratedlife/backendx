...
func NewSSEventSource(w http.ResponseWriter) *SSEventSource {
...
func (s *SSEventSource) Send(ctx context.Context, data []byte) error {
	...
	fmt.Fprintf(s.ResponseWriter, "event: message\n")
...
func (s *SSEventSource) Wait() {
	<-s.Request.Context().Done()
}
...
func (c *StudentApiController) SearchAuctionsStudent(w http.ResponseWriter, r *http.Request) {
	...
	// Create a new SSE event source.
	eventSource := NewSSEventSource(w)
	...
	go func() {
		for {
			...
			err = eventSource.Send(context.Background(), marshal)
			if err != nil {
				c.errorHandler(w, r, err, &result)
				return
			}
